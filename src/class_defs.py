...
class Point:
    ...
    def solve_p_t(self, t_step, input_values, left_point, right_point, source):
        ...
        # if in the time step zero, previous values don't exist
        if t_step == 0:
            p_prev = 0
        else:
            p_prev = self.get_p_t()[t_step - 1]

        # if the current point is the point source, calculate source function
        if self.get_id() == source.get_source_id():
            source_current = source.s_t(t_step * input_values.get_dt())
        else:
            source_current = 0

        # get current middle, left and right p
...
class Source:
    ...
    # initialization function
    def __init__(self, source_id, freq):
        self.source_id = source_id
        self.freq = freq

    # getters for the source index
    ...
    def s_t(self, t):
        t0 = 0.04
        magnitude = -8 * self.freq * (t - t0) * math.exp(-pow(4 * self.freq, 2) * pow(t - t0, 2))
        if abs(magnitude) >= 10e-16:
            return magnitude
        else:
            return 0
